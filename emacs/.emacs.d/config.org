...
Some awesome customisations stolen from [[http://www.holgerschurig.de/en/emacs-tayloring-the-built-in-mode-line/][this blog post]].
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;; XXX this is trivial to do myself using winner-mode and `same-window-p'
...
#+BEGIN_SRC emacs-lisp
 ...
 (use-package taskpaper-mode
    :bind
    ("C-c a" . taskpaper-agenda-search)
    ("C-c s" . taskpaper-agenda-select)
    :config
    (setq taskpaper-agenda-files "~/Library/Mobile Documents/com~hogbaysoftware~TaskPaper/Documents"
          taskpaper-agenda-restore-windows-after-quit t))
#+END_SRC

** highlighting changes
Highlight unsaved changes in ~prog-mode~ buffers.
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'prog-mode-hook 'highlight-changes-remove-after-save)
#+END_SRC
